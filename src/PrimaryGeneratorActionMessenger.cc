...
PrimaryGeneratorActionMessenger::PrimaryGeneratorActionMessenger(PrimaryGeneratorAction* stack):
...
void PrimaryGeneratorActionMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
    ...
    if (command == fPrimaryRelSigmaEnergyCmd) {
        fPrimaryGeneratorAction->SetRelSigmaEnergy(fPrimaryRelSigmaEnergyCmd->GetNewDoubleValue(newValue));
    }

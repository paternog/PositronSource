...
radius_array=($(seq 1.5 0.1 2.0)) #mm
#radius_array=(1.5) #mm
...
n_D=${#D_array[@]}
n_radius=${#radius_array[@]}

n_runs=$(($n_t * $n_t_conv * $n_D * $n_radius))
...
do
	...
	do
		for (( k=0; k<$n_D; k++ ))
		do
			for (( p=0; p<$n_radius; p++ ))
			do
				echo Running simulation $(($count + 1)) at $energy with: rad_th "=" ${t_array[$i]} mm, D "=" ${D_array[$k]} cm, conv_th "=" ${t_conv_array[$j]} mm , sphere_radius "=" ${radius_array[$p]}...
				...
				sed -i "s+/det/setSphereRadius .*+/det/setSphereRadius ${radius_array[$p]} mm+" $macro
				sed -i "s+/det/setRadiatorConverterSepDistance .*+/det/setRadiatorConverterSepDistance ${D_array[$k]} cm+" $macro			
					
				sed -i "s+/run/setfilenamesave output/output_${energy}_W.*+/run/setfilenamesave output/output_${energy}_W${t_array[$i]}mm_D${D_array[$k]}cm_GT${t_conv_array[$j]}mm_spheres${radius_array[$p]}mm.root+" $macro		
				...
				sed -i "s+/score/dumpQuantityToFile boxMesh1 Edep output/Edep_${energy}_W.*+/score/dumpQuantityToFile boxMesh1 Edep output/Edep_${energy}_W${t_array[$i]}mm_D${D_array[$k]}cm_GT${t_conv_array[$j]}mm_spheres${radius_array[$p]}mm.txt+" $macro		
				sed -i "s+/score/dumpQuantityToFile boxMesh2 Edep output/Edep_${energy}_W.*+/score/dumpQuantityToFile boxMesh2 Edep output/Edep_${energy}_W${t_array[$i]}mm_D${D_array[$k]}cm_GT${t_conv_array[$j]}mm_spheres${radius_array[$p]}mm_rad.txt+" $macro		
				sleep 1
				
				#RUN YOUR MACRO HERE 
				./positronSource $macro $Ncores > output/output_${energy}_W${t_array[$i]}mm_D${D_array[$k]}cm_GT${t_conv_array[$j]}mm_spheres${radius_array[$p]}mm.out
